...
class Suplayer extends CI_Controller
{
	...
	public function proc_upload()
	{
		if (!empty($_FILES['doc']['name'])) {
			$file = $this->suplayer_model->upload($_FILES['doc']);
	...
	public function insert()
	{
		if (!empty($_POST['file'])) {
			$file = $_POST['file'];
			$file = FCPATH . 'assets/images/modules/suplayer/' . $file;
			...
			if (!empty($data)) {
				if ($this->db->insert_batch('suplayer', $data)) {
	...
	{
		$data = $this->db->list_fields('suplayer');
		unset($data[0]);
		$alp = alphabet();
		$tot = count($data);
		...
		header('Content-Disposition: attachment;filename="suplayer.xlsx"');
	...
	{
		$data = $this->suplayer_model->save();
		$data['data'] = $this->suplayer_model->all();
		$this->load->view('index', ['data' => $data]);
	}

	public function edit($id = 0)
	{
		$data = $this->suplayer_model->save($id);
		$this->load->view('index', ['data' => $data]);
	}
	public function delete($id = 0)
	{
		if (!empty($id)) {
			$data = $this->suplayer_model->delete($id);
			$this->load->view('index', ['data' => $data]);
		}
	}
}
